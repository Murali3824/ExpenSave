...
import { LogIn, HandCoins, ChevronDown } from 'lucide-react'; // Import the dropdown icon (ChevronDown)
        ...
        <nav className="w-full absolute top-0 z-50">
            <div className="mx-auto px-4 md:px-14 lg:px-24">
                <div className="flex justify-between items-center py-6">
                    ...
                    {userData ? (
                        <div className="relative flex items-center gap-1 rounded-3xl bg-indigo-400 px-1 sm:px-2">
                            {/* Display the user's first letter */}
                            <div
                                className="cursor-pointer w-8 h-9 sm:w-12 sm:h-12 text-3xl font-bold flex justify-center items-center bg-indigo-400 text-white rounded-l-3xl"
                            ...
                            <div onClick={toggleDropdown}>
                                <ChevronDown
                                    className={`w-5 h-6 sm:w-7 sm:h-8 cursor-pointer text-white transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`}
                                />
                            </div>
